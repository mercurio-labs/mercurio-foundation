...
use mercurio_model::MetamodelAttributeRegistry;
use mercurio_model::{Element, Graph, GraphError};
use mercurio_runtime::{Atom, Term};
use mercurio_semantic_services::identity::{SourceSpanRef, stable_digest};
...
mod tests {
    ...
    fn test_ai_context_usage() -> mercurio_semantic_services::mutation::AiSemanticContextUsage {
        mercurio_semantic_services::mutation::AiSemanticContextUsage {
    ...
    fn design_intent_drives_assessment_goal_and_quality_gate_contracts() {
        ...
        let assessment_result = crate::assessment::run_runtime_assessment(
            crate::assessment::RuntimeAssessmentRequest {
                ...
                rulepacks: Vec::new(),
                facts: vec![
                    mercurio_runtime::Fact::new("design_intent_summary", [intent.summary.clone()]),
                    mercurio_runtime::Fact::new("design_intent_goal", [intent.goals[0].clone()]),
                ],
            },
        )
        .unwrap();
        ...
        let context = mercurio_semantic_services::mutation::SemanticReasoningContext {
            schema_version:
                mercurio_semantic_services::mutation::AI_SEMANTIC_CONTEXT_SCHEMA_VERSION.to_string(),
            metamodel_version: "test".to_string(),
            workspace_revision: mercurio_semantic_services::mutation::WorkspaceRevision::unchecked(
            ),
            focus: Vec::new(),
            elements: vec![
                mercurio_semantic_services::mutation::SemanticElementContext {
                    element: mercurio_semantic_services::mutation::ElementRef::new(
                        "Demo.ReduceLosses",
                    ),
                    kind: "definition".to_string(),
                    label: "ReduceLosses".to_string(),
                    owner: Some(mercurio_semantic_services::mutation::ElementRef::new(
                        "Demo",
                    )),

...
pub use mercurio_semantic_services::mutation::SemanticElementRef;
use mercurio_semantic_services::mutation::WorkspaceRevision;
...
use mercurio_workspace::model_state::{ModelBuildRecord, ModelRevision, ModelRevisionProducer};
...
impl SemanticWorkspaceSnapshot {
    ...
    pub fn element(&self, element_id: &str) -> Option<&mercurio_model::Element> {
...
fn element_inspection_payload(
    workspace: &SemanticWorkspaceSnapshot,
    element: &mercurio_model::Element,
) -> Value {
...
fn inspection_match_rank(element: &mercurio_model::Element, terms: &[String]) -> u8 {
    if terms
        .iter()
...
fn element_ref(element: &mercurio_model::Element) -> SemanticElementRef {
    SemanticElementRef::from_graph_element(element)
}

fn source_span_for_element(element: &mercurio_model::Element) -> Option<SourceSpanRef> {
...
fn string_property(element: &mercurio_model::Element, key: &str) -> Option<String> {
    element
        .properties
        .get(key)
        .and_then(Value::as_str)
...
fn bool_property(element: &mercurio_model::Element, key: &str) -> Option<bool> {
    element
        .properties
        .get(key)
...
fn is_authored_model_element(element: &mercurio_model::Element) -> bool {
...
fn is_metamodel_element(element: &mercurio_model::Element) -> bool {
    string_property(element, "metamodel_layer").is_some()
        || string_property(element, "metamodel_language").is_some()
        || matches!(element.kind.as_ref(), "Metaclass" | "MetamodelFeature")
...
fn is_stdlib_element(element: &mercurio_model::Element) -> bool {
...
fn value_digest(value: &Value) -> String {
    ...
    mercurio_semantic_services::stable_digest([("semantic-artifact".as_bytes(), bytes.as_slice())])
}

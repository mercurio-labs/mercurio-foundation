    ...
    if element.kind.contains("Import") || id.starts_with("import.") {
        let path = element
            ...
            .and_then(|value| match value {
                ...
                Value::Array(targets) if targets.len() == 1 => targets[0].as_str(),
                _ => None,
            })
...
mod tests {
    ...
    #[test]
    fn kir_import_reconstruction_accepts_compiler_targets_and_legacy_paths() {
        for (target, expected) in [
            (json!(["ScalarValues::*"]), "ScalarValues::*"),
            (json!(["type.Parts.Chassis"]), "Parts::Chassis"),
            (json!("Parts::Chassis"), "Parts::Chassis"),
        ] {
            let element = KirElement {
                id: "import.Demo.1".to_string(),
                kind: "Import".to_string(),
                layer: 2,
                properties: BTreeMap::from([("imports".to_string(), target)]),
            };
            let by_id = std::collections::HashMap::from([(element.id.clone(), element)]);
            let declaration =
                super::build_declaration_from_kir("import.Demo.1", &by_id, &mut Default::default())
                    .unwrap()
                    .unwrap();
            let super::Declaration::Import(import) = declaration else {
                panic!("expected an import declaration");
            };
            assert_eq!(import.path.as_colon_string(), expected);
        }
    }

    #[test]
    fn kir_import_reconstruction_rejects_ambiguous_or_empty_targets() {
        for target in [json!([]), json!(["A", "B"]), json!([null]), json!("")] {
            let element = KirElement {
                id: "import.Demo.1".to_string(),
                kind: "Import".to_string(),
                layer: 2,
            ...
            let by_id = std::collections::HashMap::from([(element.id.clone(), element)]);
            assert!(super::build_declaration_from_kir(
                "import.Demo.1", &by_id, &mut Default::default(),
            ).is_err());
        }
    }
